...
def queue_create():
	...
	q = conn.create_queue("C13765235-%s" % body['name'])
...
def queue_msgcount(queue):
	...
	curl -X GET -H 'Content-Type: application/json' http://localhost:5000/queue/<mytestqueue>/msqs/count
	"""
...
@app.route("/queues/<queue>/msgs", methods=["GET"])
def queue_msgread(queue):
	"""
	Reads a message to the queue
	
	curl -X GET -H 'Content-Type: application/json' http://localhost:5000/queue/<mytestqueue>/msqs
	"""
	conn = get_conn()
